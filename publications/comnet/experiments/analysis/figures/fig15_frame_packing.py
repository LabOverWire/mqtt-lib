...
def main(hol_dir: Path, output_dir: Path):
    apply_style()

    v3_pertopic = load_results(hol_dir, ["quic-pertopic"])
    v3_perpub = load_results(hol_dir, ["quic-perpub"])
    v4_pertopic = load_results(hol_dir, ["quic-pertopic-isolated"])
    v4_perpub = load_results(hol_dir, ["quic-perpub-isolated"])
...
if __name__ == "__main__":
    ...
    hol_dir = Path(sys.argv[1]) if len(sys.argv) > 1 else base / "results-v5" / "02_hol_blocking"
    output_dir = Path(sys.argv[2]) if len(sys.argv) > 2 else script_dir / "output"
    output_dir.mkdir(parents=True, exist_ok=True)
    main(hol_dir, output_dir)
