...
It also supports **deferred acknowledgement** (MQTT 5.0 only): `subscribe_with_ack` delivers each message with a move-only `AckToken` and withholds the PUBACK/PUBREC until the application calls `token.ack()`, so the acknowledgement means "processed" and the inbound Receive Maximum window becomes end-to-end backpressure. See `crates/mqtt5/examples/deferred_ack.rs`.
