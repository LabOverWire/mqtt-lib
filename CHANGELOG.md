...
- **Added `MqttBroker::quic_local_addr()`**, returning the address the first QUIC endpoint is bound to. This mirrors `local_addr` / `tls_local_addr` / `ws_local_addr` and lets a broker bound to port 0 report the port the OS assigned.
